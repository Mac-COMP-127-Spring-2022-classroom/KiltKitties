...
public class Gene {
    ...
    public Gene(String furTrait, String bellyTrait, String eyeTrait, String accessoriesTrait, int furMaxLevel, int bellyMaxLevel, int eyeMaxLevel, int accessoriesMaxLevel) {
        this.furGene.put("ShadowGrey", List.of(177, 177, 190));
        this.furGene.put("Salmon", List.of(244, 167, 146));
        this.furGene.put("Meowgarine", List.of(252, 252, 149));
        this.furGene.put("OrangeSoda", List.of(247, 188, 86));
        ...
        for (Integer level: levelList) {
            if (level != 35) {
                ...
                furTraits.put(level, furTrait);
            }
        }
        for (Integer level: levelList) {
            if (level != 35) {
                bellyTraits.put(level, getRandomBellyGeneLevel(rand.nextInt(furMaxLevel + 1)));
            } else if (level == 35) {
                bellyTraits.put(level, bellyTrait);
            }
        }
        for (Integer level: levelList) {
            if (level != 35) {
                ...
                eyeTraits.put(level, eyeTrait);
            }
        }
        for (Integer level: levelList) {
            if (level != 35) {
                accessoriesTraits.put(level, getRandomAccessoriesGeneLevel(rand.nextInt(furMaxLevel + 1)));
            } else if (level == 35) {
                accessoriesTraits.put(level, accessoriesTrait);
            }
        }
        System.out.println(furTraits);
        System.out.println(bellyTraits);
    ...
    public static void main(String[] args) {
        Gene gene = new Gene("Onyx", "Daffodil", "Gold", "Bow", 2, 2, 1, 1);
    }
}
